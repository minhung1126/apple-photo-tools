...
6. 日常照片一律加上時間前綴，方便長期依檔名排序與歸檔；時間優先取內嵌 metadata，沒有就退回檔案時間。
...
ExifTool 可以讓日常檔案優先使用媒體內嵌時間：

```bash
brew install exiftool
```

沒有 ExifTool 也可以執行；日常仍然會整理，只是時間會退回 filesystem 建立時間，必要時再退回修改時間。
...
都視為主題 / 日期事件資料夾，不做一般重新命名。

`20260910QWER`（日期後沒有空格）不符合目前辨識格式，會被忽略。
...
- 決定根目錄散圖要進哪個 `YYYYMM00`
- 產生 `YYYYMMDD-HHMMSS_原始檔名.ext`

時間優先序固定為：

1. **既有檔名時間前綴**：如果已經是 `YYYYMMDD-HHMMSS_...`，優先沿用，確保重跑時不改月份。
2. **內嵌媒體 metadata**：透過 ExifTool 讀 `DateTimeOriginal`、`CreateDate`、`MediaCreateDate`、`TrackCreateDate`。
3. **filesystem birth / creation time**。
4. **filesystem modification time**：最後 fallback。

如果使用到第 3 或第 4 層，Terminal 會顯示 `[WARN]`，讓你知道這不是內嵌拍攝時間。

## 為什麼保留 filesystem fallback

因為日常的定義就是：

> **沒有被你手動放進主題資料夾的照片 / 影片，都屬於日常，而且日常檔名一定要有時間前綴。**

因此截圖、下載圖片、沒有 EXIF 的檔案，也不能因為缺少 `DateTimeOriginal` 就留成未整理狀態。
...
```text
20260925-150000_20260925-142530_IMG_1234.HEIC
```

如果它被放回根目錄，月份分類也會優先使用檔名裡的 `20260925-142530`，而不是新的複製 / 下載時間。

## 同組檔案

同一 basename 的檔案共用一個時間，例如 Live Photo：
...
Stage 3 會先用該原始媒體決定時間，再處理編輯後主檔。

## 主題完全不同

`YYYYMMDD 主題` 不進行這套一般重新命名。

不論檔案來源或有沒有 metadata，都保持原檔名。
...
都會加上時間前綴；優先使用 metadata，沒有則依統一 fallback 規則取得檔案時間。
...
沒有內嵌 metadata 時會退回檔案建立時間；建立時間不可用時，再退回修改時間並警告。

只有連 filesystem 時間都無法讀取的異常檔案才會跳過。

## 8. 同組目的檔碰撞會整組跳過

除了檢查既有檔案，程式也會預先檢查同一組內是否有兩個來源將產生相同目的檔名（包含大小寫不敏感檔案系統的情況）。

若碰撞，整組跳過，不嘗試覆寫。

## 9. Originals 不會再次自動整理
...
- `.command` zsh 語法正確，而且 syntax step 為 fail-fast
- executable 權限存在
- 根目錄散圖進入當月 `YYYYMM00`
...
- 日常中含空格的非標準檔名可正常處理
- 沒有 EXIF 的標準 `IMG_####.JPG` 也會使用檔案時間完成日常整理
- 已有 `YYYYMMDD-HHMMSS_` 前綴的檔案不會重複改名
- 已命名檔案從根目錄重新分類時優先沿用檔名前綴月份
